...
// Virtual for counting contents
BankSchema.virtual("contentsCount", {
  ref: "Content",
  localField: "_id",
  foreignField: "bank",
  count: true,
...
// Virtual for total questions count through contents and groups
...
BankSchema.pre("findOneAndDelete", async function (next) {
  try {
    const bank = await this.model.findOne(this.getFilter());
    if (bank) {
      // نحذف جميع الـ Contents المرتبطة بهذا البنك
      await Content.deleteMany({ bank: bank._id });
      console.log(`✅ تم حذف جميع المحتويات المرتبطة بالبنك ${bank._id}`);
    }
    next();
  } catch (error) {
    next(error as Error);
